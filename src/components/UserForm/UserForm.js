...
function UserForm({ onSave, onClose, user }) {
		...
		useEffect(() => {
				if (user) {
						setFormData({
								...user,
								dataNasc: user.dataNasc ? dayjs(user.dataNasc, ['YYYY-MM-DD', 'DD/MM/YYYY']).isValid() ? dayjs(user.dataNasc, ['YYYY-MM-DD', 'DD/MM/YYYY']) : null : null
						});
				}
		}, [user]);
		...
		const handleSubmit = (e) => {
				...
				if (Object.values(newErrors).every((error) => error === '')) {
						onSave({
								...formData,
								dataNasc: formData.dataNasc ? formData.dataNasc.format('DD/MM/YYYY') : ''
						});
				}
		};
